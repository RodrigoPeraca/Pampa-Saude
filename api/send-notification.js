  ...
  try {
    ...
    const messages = tokens.map(({ token }) => ({
      ...
      notification: {
        title,
        body,
        // image é opcional — só inclui se existir
        ...(image && { imageUrl: image }),
      },
      // data garante compatibilidade com Android e service workers mais antigos
      data: {
        title,
        body,
        ...(image && { image }),
    ...
    response.responses.forEach((resp, index) => {
      if (!resp.success) {
        ...
        if (
          ...
          batch.delete(db.collection("fcm_tokens").doc(tokens[index].id));
        }
      }
    });
    await batch.commit();
